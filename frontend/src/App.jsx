...
function RegisterScreen({ onLogin, onGoLogin }) {
  const [form, setForm] = useState({ nome: "", email: "", senha: "", perfil: "paciente", crm: "", especialidade_id: 0, telefone_profissional: "" });
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        {form.perfil === "medico" && <>
          <div className="field"><label>CRM</label><input value={form.crm} onChange={e => setForm(f => ({ ...f, crm: e.target.value }))} placeholder="CRM-12345" /></div>
          <div className="field">
            <label>Especialidade</label>
            <select value={form.especialidade_id} onChange={e => setForm(f => ({ ...f, especialidade_id: parseInt(e.target.value) }))}>
              <option value={0}>Selecione...</option>
              {especialidades.map(e => <option key={e.id} value={e.id}>{e.nome}</option>)}
            </select>
          </div>
          <div className="field"><label>Telefone profissional</label><input value={form.telefone_profissional} onChange={e => setForm(f => ({ ...f, telefone_profissional: e.target.value }))} /></div>
        </>}
...
function ReagendarModal({ consulta, token, onClose, onDone }) {
  ...
  useEffect(() => {
    apiFetch(`scheduling/horarios/${consulta.medico_id}`, {}, token)
      .then(data => setHorarios(data.filter(h => h.disponivel)))
      .catch(() => {});
  }, []);
  ...
  async function confirmar() {
    setErr(""); setLoading(true);
    try {
      await apiFetch(`scheduling/consultas/${consulta.id}/remarcar`, {
        method: "PUT",
        body: JSON.stringify({ novo_horario_id: parseInt(novoHorario) })
      }, token);
      onDone();
    } catch (e) { setErr(e.message); }
    finally { setLoading(false); }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
      <div className="card" style={{ width: 400, maxWidth: "90vw" }}>
        <div className="card-title">Reagendar consulta</div>
        <Alert msg={err} />
        <div className="field">
        ...
        <div className="actions">
          <button className="btn btn-primary" style={{ width: "auto" }} disabled={!novoHorario || loading} onClick={confirmar}>
    ...
    <div className="container">
      ...
      {tab === "consultas" && (
        <div className="card">
                ...
                <tbody>
                  {consultas.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>
                        {c.status === "agendada" && <div className="actions">
                          <button className="btn btn-outline btn-sm" onClick={() => setReagendando(c)}>Reagendar</button>
          ...
          {reagendando && <ReagendarModal consulta={reagendando} token={user.token} onClose={() => setReagendando(null)} onDone={() => { setReagendando(null); carregarConsultas(); }} />}
        </div>
      )}
    </div>
  );
...
function MedicoHorarios({ user }) {
  ...
  async function carregar() {
    try {
      const data = await apiFetch("scheduling/horarios/meus", {}, user.token);
      setHorarios(data);
    } catch { setHorarios([]); }
  }
